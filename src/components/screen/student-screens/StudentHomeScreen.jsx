...
const StudentHomeScreen = () => {
  ...
  const fetchSchedule = async () => {
    ...
    // setLoading(true); // Tùy chọn: bật loading ở đây hoặc ở useEffect

    try {
      // Lấy lịch trong khoảng 1 năm (tháng hiện tại -> năm sau)
      ...
      if (rawList.length > 0) {
        ...
        rawList.forEach((item) => {
          ...
          // Format giờ hiển thị (Nếu backend đã trả về timeString thì dùng luôn, không thì format lại)
          const timeDisplay = item.timeString 
            ? item.timeString 
            : `${moment(item.startTime).format("HH:mm")} - ${moment(item.endTime).format("HH:mm")}`;

          // Địa điểm: Ưu tiên API -> Fallback Redux
          const addressDisplay = item.address || defaultAddress;

          formattedSchedule[dateKey].push({
            id: item.id,
            
            // Với Teacher: Quan trọng là dạy LỚP NÀO (VD: IT-K18)
            name: item.className || "Lớp học", 
            // Có thể thêm tên môn học để hiển thị phụ đề
            courseName: item.courseName, 

            time: timeDisplay,
            address: addressDisplay,
            
            // Các trạng thái quản lý lớp học
            status: item.sessionStatus, // SCHEDULED, ONGOING...
            isAttendanceOpen: item.isAttendanceOpen, // Quan trọng: Để hiện nút "Đóng/Mở điểm danh"
          });
        });

        setScheduleData(formattedSchedule);
        
        // --- LOGIC CHỌN NGÀY THÔNG MINH ---
        const listDates = Object.keys(formattedSchedule).sort();
        
        // Nếu hôm nay có lịch -> giữ nguyên (Calendar mặc định chọn today)
        // Nếu hôm nay KHÔNG có lịch -> Chọn ngày có lịch GẦN NHẤT trong tương lai
        if (listDates.length > 0 && !formattedSchedule[today]) {
             const upcomingDate = listDates.find(date => moment(date).isSameOrAfter(today));
             if (upcomingDate) {
                 setSelectedDate(upcomingDate);
             } else {
                 // Nếu hết lịch tương lai, chọn ngày cuối cùng trong quá khứ
                 setSelectedDate(listDates[listDates.length - 1]);
             }
        }
      } else {
        setScheduleData({}); // Xóa dữ liệu cũ nếu API trả về rỗng
      }
    } catch (error) {
      console.error("❌ Lỗi lấy lịch giảng dạy:", error);
    } finally {
      setLoading(false);
    }
  };
